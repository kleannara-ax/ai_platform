(function(){
  const HOST_ID = 'fwRcvModalHost';
  const FRAME_ID = 'fwRcvModalFrame';
  const CLOSE_ID = 'fwRcvModalClose';
  ...
  function notifyClose(request, reason) {
    ...
    document.dispatchEvent(new CustomEvent('fireweb:receiver-modal-close', {
      detail: {
        reason,
        mode: request.mode,
        options: request.options
  ...
  function ensureHost() {
    ...
    modalEl.innerHTML = [
      ...
      `      <iframe id="${FRAME_ID}" title="수신기 작업" style="width:100%;height:100%;border:0;background:transparent;display:block;"></iframe>`,
      '    </div>',
      '  </div>',
      '</div>'
    ].join('');
  ...
  function open(options) {
    ...
    closingReason = null;
    frame.src = `/receivers.html?${buildQuery(options).toString()}`;
    activeRequest = {
  ...
  window.FireWebReceiverModal = { open };
})();
