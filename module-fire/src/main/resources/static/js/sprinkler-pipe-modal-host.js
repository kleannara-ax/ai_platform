(function(){
  const HOST_ID = 'fwSprinklerPipeModalHost';
  const FRAME_ID = 'fwSprinklerPipeModalFrame';
  const CLOSE_ID = 'fwSprinklerPipeModalClose';
  ...
  function notifyClose(request, reason) {
    ...
    document.dispatchEvent(new CustomEvent('fireweb:sprinkler-pipe-modal-close', {
      detail: {
        reason,
        mode: request.mode,
        options: request.options
  ...
  function ensureHost() {
    ...
    modalEl.innerHTML = [
      ...
      `      <iframe id="${FRAME_ID}" title="스프링쿨러 배관 작업" style="width:100%;height:100%;border:0;background:transparent;display:block;"></iframe>`,
      '    </div>',
      '  </div>',
      '</div>'
    ].join('');
  ...
  function open(options) {
    ...
    closingReason = null;
    frame.src = `/sprinkler-pipes.html?${buildQuery(options).toString()}`;
    activeRequest = {
  ...
  window.FireWebSprinklerPipeModal = { open };
})();
