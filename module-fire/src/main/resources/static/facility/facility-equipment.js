(() => {
        ...
        <section class="card summary-gradient-card mb-4">
          <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-3 text-white">
            <div>
              ...
              <div class="text-white-50 fw-semibold">${esc(CFG.subtitle || '')}</div>
        ...
        <section class="card shadow-sm mb-4">
          <div class="card-body">
            <div class="row g-2 align-items-center">
              <div class="col-lg-4"><input id="filterQ" class="form-control" placeholder="검색어(건물/층/종류/비고)"></div>
  ...
  function drawSingleMarker(wrapId, imgId, layerId, x, y, icon) {
    ...
    const draw = () => {
      ...
      const m = document.createElement('div'); m.style.cssText = `position:absolute;left:${ox+w*nx/100}px;top:${oy+h*ny/100}px;width:28px;height:28px;transform:translate(-50%,-50%);filter:drop-shadow(0 0 6px rgba(0,0,0,.6))`;
      m.innerHTML = `<img src="${esc(icon)}" alt="" style="width:100%;height:100%;object-fit:contain">`; layer.appendChild(m);
    };
