...
-- ============================================================
-- module-ps-insp: PS_INSP_AUTH 권한자를 사용자별 코드값으로 전환
--
-- 기존: PPM_ADMIN.EXTRA_VALUE1 = 로그인 ID 목록(콤마 구분)
-- 변경: PS_INSP_AUTH 하위의 활성 CODE 한 건이 권한자 로그인 ID 한 명을 의미
--         CODE_NAME은 공통코드 관리 화면에서 표시할 사용자명으로 관리
-- ============================================================

DELIMITER //

CREATE PROCEDURE migrate_ps_insp_auth_to_user_codes()
BEGIN
    ...
    SELECT g.GROUP_ID, COALESCE(d.EXTRA_VALUE1, '')
     ...
     WHERE g.GROUP_CODE = 'PS_INSP_AUTH'
     LIMIT 1;
        ...
        WHILE CHAR_LENGTH(TRIM(v_user_ids)) > 0 DO
            ...
            IF v_user_id <> '' THEN
                INSERT INTO code_detail (
                    ...
                    'PS 지분검사 PPM 기준값 수정 권한 사용자 로그인 ID', NULL, NULL, 1, 100
                )
...
CALL migrate_ps_insp_auth_to_user_codes() //
DROP PROCEDURE migrate_ps_insp_auth_to_user_codes //
