...
-- ============================================================
-- V38: OTHER_PERM 권한자를 사용자별 코드값으로 전환
--
-- 기존: OTHER_ADMIN.EXTRA_VALUE1 = 로그인 ID 목록(콤마 구분)
-- 변경: OTHER_PERM 하위의 활성 CODE 한 건이 권한자 로그인 ID 한 명을 의미
--         CODE_NAME은 공통코드 관리 화면에서 표시할 사용자명으로 관리
-- ============================================================

DELIMITER //

CREATE PROCEDURE migrate_other_perm_to_user_codes()
BEGIN
    ...
    SELECT g.GROUP_ID, COALESCE(d.EXTRA_VALUE1, '')
      ...
      LEFT JOIN code_detail d
       ...
       AND d.CODE = 'OTHER_ADMIN'
     WHERE g.GROUP_CODE = 'OTHER_PERM'
     LIMIT 1;
    ...
    IF v_group_id IS NOT NULL THEN
        WHILE CHAR_LENGTH(TRIM(v_user_ids)) > 0 DO
            ...
            IF v_user_id <> '' THEN
                INSERT INTO code_detail (
                    ...
                    '기타시설관리 권한 사용자 로그인 ID', NULL, NULL, 1, 100
                )
        ...
        UPDATE code_detail
         ...
         WHERE GROUP_ID = v_group_id
           AND CODE = 'OTHER_ADMIN';
    END IF;
END //

CALL migrate_other_perm_to_user_codes() //
DROP PROCEDURE migrate_other_perm_to_user_codes //
