...
- **설비관리시스템 공통/QR 화면**:
  ...
  - `/qr` → `설비관리시스템 > QR코드` 공통 메뉴에서 QR코드 발급/확인 화면 로드. 좌측 설비 선택 카드 안에서 `소방설비`/`기타설비`를 전환하며, 소방설비는 소화기·소화전·수신기·소방펌프·스프링클러 QR을 표시하고 기타설비는 다음 단계 QR 연결 전까지 에어컨·정수기 선택 버튼만 표시
- **소방설비 화면**:
...
- QR코드 화면의 좌측 설비 선택 카드 안에 `소방설비`/`기타설비` 전환 버튼을 추가했습니다. `소방설비` 선택 시 기존 소화기·소화전·수신기·소방펌프·스프링클러 목록을 유지하고, `기타설비` 선택 시 다음 단계 QR 연결 전까지 에어컨·정수기 버튼과 준비 안내만 표시합니다.
