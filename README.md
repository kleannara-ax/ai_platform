...
- **Main Features**:
  ...
  - 층별 도면: 설비 구분을 페이지 최상단으로 분리하고, 그 아래 건물/층/선택설비 컨트롤 카드와 대형 도면 카드를 배치한 반응형 UI입니다. 컨트롤 카드의 `층별 도면` 제목/설명과 도면 안 설비 목록은 표시하지 않습니다. 도면 카드 상단은 좌측 `도면`, 우측 `확대 초기화` 버튼만 표시하며, 소화기·소화전·스프링클러·에어컨·정수기 마커를 전환 표시합니다. 옥외 도면에서는 수신기·소방펌프를 추가 노출하며, 삭제는 상단 버튼이 아니라 마커 클릭 시 표시되는 미니 모달 안에서만 수행합니다.
...
- **설비관리시스템 공통 도면 화면**:
  ...
  - `/maps/floor.html` → `설비관리시스템 > 층별 도면` 공통 메뉴에서 로드. 최상단 설비 구분 드롭다운, 다음 카드의 건물/층/선택설비 드롭다운, 도면 카드 좌측 `도면` 텍스트와 우측 `확대 초기화` 버튼을 표시하고 도면 내 목록은 표시하지 않음. `domainType=fire|other`, `assetType=ext|hyd|sprinkler|receiver|pump|aircon|water`, `buildingName`, `floorName`, `focusType`, `focusId` 지원
- **소방설비 화면**:
...
6. 층별 도면에서는 맨 위의 `설비 구분`을 먼저 선택하고, 아래 컨트롤 카드에서 `건물`, `층`, `선택설비`를 선택합니다. 소방설비는 소화기/소화전/스프링클러를 표시하고, 옥외 도면에서는 수신기/소방펌프도 선택설비에 추가됩니다. 기타설비를 선택하면 에어컨/정수기만 표시됩니다. 도면 카드는 좌측 `도면` 제목과 우측 `확대 초기화` 버튼을 제공하며, 도면 영역은 목록 없이 크게 표시됩니다. `설비 추가`는 현재 선택설비를 현재 도면에서 클릭한 위치에 추가합니다. 설비 삭제는 상단 버튼이 아니라 마커 클릭 시 나타나는 미니 모달의 `삭제` 버튼으로 수행하고, 저장/점검/삭제 후 마커가 갱신됩니다.
...
- 층별 도면을 최상단 `설비 구분` 드롭다운, 별도 `건물 / 층 / 선택설비` 컨트롤 카드, 대형 도면 카드 구조로 재구성하고, 컨트롤 카드의 `층별 도면` 제목/설명과 도면 내부 목록 및 상단 삭제/점검/수정류 버튼을 제거. 도면 카드 상단에는 좌측 `도면` 텍스트와 우측 `확대 초기화` 버튼만 표시합니다. 도면이 등록된 건물/층만 표시하며, 소방설비는 소화기·소화전·스프링클러를 표시하고 옥외 도면에서만 수신기·소방펌프를 추가 노출합니다. 기타설비는 에어컨·정수기 마커와 추가/수정/점검 iframe 연동을 지원하며, 삭제는 마커 클릭 미니 모달에서만 수행합니다.
