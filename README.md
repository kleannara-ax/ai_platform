...
- **Main Features**:
  ...
  - 소화기 목록 총 건수는 QR 코드 관리 화면과 동일하게 `extinguisher` 엔티티 row 기준(`Page.totalElements`)으로 집계하며, 고정 200건 로딩 제한으로 인한 QR 수량 불일치를 방지
...
- 소화기 목록 화면의 총 건수가 첫 페이지 로딩 건수(`content.length`)가 아니라 API의 전체 row 수(`totalElements`)를 사용하고, 필요 시 추가 페이지를 순차 로딩하도록 개선하여 QR 코드 관리 화면의 소화기 수와 동일한 기준으로 표시
