...
- **Main Features**:
  ...
  - 기타설비 대시보드/도면: 에어컨·정수기 현황 카드, 상태 그래프, 이상설비 목록, 메인 도면, 층별 도면 마커/정보 카드 제공(옥외 설비 흐름 제외)
...
- **기타설비 화면**:
  - `/index.html?page=other_dashboard` → 기타설비 대시보드(에어컨/정수기 현황, 점검필요/교체필요/이상설비, 층별 도면 바로가기)
  - `/facility-map.html` → 기타설비 도면(메인): 사업장 메인 도면에서 건물/층 선택 후 층별 도면 이동, 옥외 설비 표시 없음
  - `/facility/floor.html` → 기타설비 층별 도면(에어컨/정수기 마커, `buildingName`, `floorName`, `buildingId`, `floorId`, `focusType=aircon|water`, `focusId` 지원)
...
6. 기타설비는 기타설비 메뉴의 대시보드, 도면(메인), 층별 도면, 에어컨, 정수기 화면에서 관리합니다. 기타설비 도면은 에어컨/정수기만 대상으로 하며 옥외 설비 선택/표시는 제공하지 않습니다.
7. 기타설비 대시보드의 이상설비 바로가기를 누르면 `/facility/floor.html`에서 해당 에어컨/정수기 마커가 자동 선택·강조되고 정보 카드가 표시됩니다.
...
- 기타설비 대시보드, 도면(메인), 층별 도면 신규 추가 완료(에어컨/정수기만 표시, 옥외 설비 제외)
...
- 에어컨/정수기 실데이터 등록 후 기타설비 대시보드/메인 도면/층별 도면 포커스 흐름 검증
