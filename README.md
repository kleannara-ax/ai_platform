...
- **Main Features**:
  ...
  - 층별 도면: 도면이 등록된 건물/층만 드롭다운으로 표시하고, 설비 구분(소방설비/기타설비)과 선택설비 드롭다운으로 소화기·소화전·스프링클러·에어컨·정수기 마커를 전환 표시합니다. 옥외 도면에서는 수신기·소방펌프를 추가 노출하며, 선택설비 기준으로 추가/삭제/점검/수정 작업을 수행합니다.
...
- **설비관리시스템 공통 도면 화면**:
  ...
  - `/maps/floor.html` → `설비관리시스템 > 층별 도면` 공통 메뉴에서 로드. 도면 등록 건물/층 드롭다운, `domainType=fire|other`, `assetType=ext|hyd|sprinkler|receiver|pump|aircon|water`, `buildingName`, `floorName`, `focusType`, `focusId` 지원
- **소방설비 화면**:
...
6. 층별 도면에서는 `설비 구분`, `건물`, `층`, `선택설비` 드롭다운을 순서대로 선택합니다. 소방설비는 소화기/소화전/스프링클러를 표시하고, 옥외 도면에서는 수신기/소방펌프도 선택설비에 추가됩니다. 기타설비를 선택하면 에어컨/정수기만 표시됩니다. `설비 추가`는 현재 선택설비를 현재 도면에서 클릭한 위치에 추가하며, 저장/점검/삭제 후 도면 목록과 마커가 갱신됩니다.
...
- 층별 도면을 `설비 구분 / 건물 / 층 / 선택설비` 드롭다운 구조로 개편하고, 도면이 등록된 건물/층만 표시하도록 개선 완료. 소방설비는 소화기·소화전·스프링클러를 표시하며 옥외 도면에서만 수신기·소방펌프를 추가 노출하고, 기타설비는 에어컨·정수기 마커와 추가/수정/점검 iframe 연동을 지원
