...
- **Main Features**:
  ...
  - 소방설비 대시보드: `/fire-dashboard.html` 별도 페이지에서 점검 필요 설비, 설비 현황 원형 그래프, 이상설비 수, 이상설비 도면/상세 바로가기 제공
  ...
  - 기타설비 대시보드/도면: 기타설비 대시보드는 `/facility-dashboard.html` 별도 페이지로 분리하고, 메인 도면은 `/fire-map.html` 원본 화면을 그대로 복사한 파생본으로 사용하되 옥외소화전/옥외 소화기/수신기·소방펌프만 제외하며, 층별 도면은 공통 DB 도면 흐름을 사용하고 에어컨은 `점검요청`, 정수기는 `점검필요` 상태만 표시
...
- **소방설비 화면**:
  - `/index.html?page=fire_dashboard` → iframe으로 `/fire-dashboard.html` 로드
  - `/fire-dashboard.html` → 소방설비 전용 대시보드
...
- **기타설비 화면**:
  - `/index.html?page=other_dashboard` → iframe으로 `/facility-dashboard.html` 로드
  - `/facility-dashboard.html` → 기타설비 전용 대시보드(에어컨/정수기 현황, 에어컨 `점검요청`, 정수기 `점검필요`, 층별 도면 바로가기)
...
2. 소방설비 대시보드는 기존 `index.html` 내부 렌더링이 아니라 `/fire-dashboard.html` 별도 페이지로 열리며, 설비 유형별 점검 필요 설비 수량과 원형 그래프를 확인합니다. 수신기/소방펌프 그래프는 정상, 점검필요, 요정비, 불량 4상태로 표시됩니다.
...
7. 기타설비 대시보드는 `/facility-dashboard.html` 별도 페이지로 열리며, 바로가기를 누르면 `/facility/floor.html`에서 해당 에어컨/정수기 마커가 자동 선택·강조되고 정보 카드가 표시됩니다. 에어컨은 `점검요청`, 정수기는 `점검필요`로 표시합니다.
...
- 소방설비/기타설비 대시보드를 각각 `/fire-dashboard.html`, `/facility-dashboard.html` 별도 페이지로 분리하고 기존 `index.html`은 메뉴/iframe 라우팅 중심으로 복구
- `/fire-dashboard.html`, `/facility-dashboard.html` 및 공통 대시보드 CSS/JS는 Security 필터 체인 우회 대상으로 보강하여 독립 정적 페이지 자체가 401로 차단되지 않도록 조치
- 이상설비 바로가기 마커 자동 선택 안정화 및 대시보드 상세 연결 완료
