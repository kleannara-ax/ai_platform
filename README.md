...
- **Main Features**:
  ...
  - 소화기 목록 총 건수는 QR 코드 관리 화면과 동일하게 `/fire-api/qr/list`의 소화기 배열 길이를 우선 기준으로 집계하며, 고정 200건 로딩 제한/브라우저 캐시로 인한 QR 수량 불일치를 방지
...
- 소화기 목록 화면의 총 건수가 첫 페이지 로딩 건수(`content.length`)가 아니라 QR 코드 관리 화면과 동일한 `/fire-api/qr/list`의 소화기 배열 길이를 우선 사용하고, 필요 시 추가 페이지를 순차 로딩하도록 개선하여 QR 코드 관리 화면의 소화기 수와 동일한 기준으로 표시
- SPA 내부 소방 모듈 iframe 로딩 시 버전 쿼리스트링을 추가해 브라우저에 캐시된 이전 `extinguishers.html`이 계속 표시되는 문제를 방지
