...
- **Main Features**:
  ...
  - 기타설비 대시보드/도면: 기존 소방설비 메인 도면/층별 도면 작업물을 기준으로 공통 도면 좌표와 DB 도면 흐름을 사용하고, 에어컨은 `점검요청`, 정수기는 `점검필요` 상태만 표시(옥외 설비 흐름 제외)
...
- **기타설비 화면**:
  ...
  - `/facility-map.html` → 기타설비 도면(메인): 기존 `/fire-map.html`의 사업장 메인 도면 좌표/구역을 기준으로 건물/층 선택 후 층별 도면 이동, 옥외 설비 표시 없음
...
6. 기타설비는 기타설비 메뉴의 대시보드, 도면(메인), 층별 도면, 에어컨, 정수기 화면에서 관리합니다. 기타설비 도면은 기존 소방설비 도면 작업물의 좌표/레이어/상호작용을 기준으로 사용하되 에어컨/정수기만 대상으로 하며 옥외 설비 선택/표시는 제공하지 않습니다.
...
- 기타설비 대시보드, 도면(메인), 층별 도면은 신규 독립 축약 구현이 아니라 기존 소방설비 도면 작업물 기준으로 재정렬 완료(공통 메인 도면 좌표, DB 층별 도면, 에어컨/정수기만 표시, 옥외 설비 제외)
