...
- **Main Features**:
  ...
  - 기타설비 관리: 에어컨, 정수기 목록/상세/등록/수정/삭제/이미지 업로드, 기타설비 QR코드 조회/인쇄, 에어컨 점검 요청, 에어컨/정수기 모바일 QR 점검 처리. 에어컨은 자동 생성 ID(`AC-000001`)와 현장 식별 No.를 별도로 관리하며, 모바일 QR 등록에서 식별 No.는 선택 입력으로 비워 둘 수 있습니다. 정수기는 기존 `WP-000001` 형식 순번 ID를 자동 생성합니다.
  - 기타설비 이미지/QR 정책: 목록에는 대표/마커 이미지를 표시하지 않고, 상세 모달에는 사진 섹션을 항상 표시해 업로드 사진이 없으면 안내 문구를 보여주며, 도면 마커는 업로드 이미지와 분리된 마커 전용 이미지만 사용. 상세 모달 QR코드는 `OTHER_PERM` 사용자 코드 권한자에게만 표시
  - 에어컨 관리 단순화: 제조사, 상세 위치, 실외기 대수(최대 2대), 제조/설치월만 관리하고 실외기 좌표/연결선, 설치연도, 수량 입력은 제거. 모바일 QR 최초 등록의 식별 No.는 선택값이며, 비워 둘 수 있음
...
- **설비관리시스템 공통/QR 화면**:
  ...
  - `/qr` → `설비관리시스템 > QR코드` 공통 메뉴에서 QR코드 발급/확인 화면 로드. QR 페이지 접속 자체는 허용하되, 좌측 `소방설비` 버튼은 `FIRE_PERM`의 활성 코드값, `기타설비` 버튼은 `OTHER_PERM`의 활성 코드값에 현재 로그인 ID가 등록된 경우에만 표시합니다. 소방설비는 소화기·소화전·수신기·소방펌프·스프링클러 QR을 표시하고 기타설비는 에어컨·정수기 등록 설비 QR을 표시. 에어컨/정수기 QR은 외부 업체용 모바일 점검 페이지로 연결합니다.
- **소방설비 화면**:
...
- **기타설비 API**:
  ...
  - `/minspection/air-conditioners/{qrKey}`: 등록 QR은 모바일 에어컨 점검(점검자 이름, 정상/비정상), 미등록 QR은 로그인 후 에어컨 신규 등록 페이지. 식별 No.는 선택 입력이며 비워 둘 수 있습니다. 등록 화면은 건물/층 선택 시 도면을 표시하고, 모바일 두 손가락 확대/축소·이동 후 탭으로 위치 좌표를 자동 저장합니다.
  - `/minspection/water-purifiers/{qrKey}`: 등록 QR은 모바일 정수기 점검(점검자 이름, 완료/미완료), 미등록 QR은 로그인 후 정수기 신규 등록 페이지. 정수기 ID 입력칸은 없고 순번 ID가 자동 생성되며, 등록 화면은 건물/층 도면 터치 좌표 선택을 지원합니다.
  - `/facility-api/mobile/air-conditioners/by-key?key={qrKey}`, `/facility-api/mobile/air-conditioners/{qrKey}/register`, `/facility-api/mobile/air-conditioners/{qrKey}/fault-reports`: 모바일 에어컨 설비 조회/QR 등록/점검 저장 API. 최초 QR 등록 POST만 JWT 인증이 필요하고, 점검 저장은 외부 업체용으로 공개 유지합니다.
  - `/facility-api/mobile/water-purifiers/by-key?key={qrKey}`, `/facility-api/mobile/water-purifiers/{qrKey}/register`, `/facility-api/mobile/water-purifiers/{qrKey}/disinfections`: 모바일 정수기 설비 조회/QR 등록/점검 저장 API. 최초 QR 등록 POST만 JWT 인증이 필요하고, 점검 저장은 외부 업체용으로 공개 유지합니다.
...
- **New Tables**:
  ...
  - `facility_equipment`: 에어컨/정수기 공통 설비 마스터. `EQUIPMENT_CODE`는 에어컨 `AC-000001` 형식의 자동 생성 설비 ID이며, 에어컨은 `EQUIPMENT_CODE`와 별개로 선택 입력인 `SERIAL_NUMBER`(식별 No.), `EQUIPMENT_TYPE`, `MANUFACTURER`, `LOCATION_DESCRIPTION`, `OUTDOOR_UNIT_COUNT`, `MANUFACTURE_DATE` 중심으로 관리합니다. 에어컨 모바일 QR 등록에서 식별 No.를 비우면 `SERIAL_NUMBER`은 `NULL`로 저장되어 화면에 공란으로 표시됩니다. 정수기는 별도 `EQUIPMENT_CODE`를 사용하지 않고 기존 `SERIAL_NUMBER`의 `WP-000001` 형식 순번 ID를 자동 생성합니다. `INSTALLATION_YEAR`, `OUTDOOR_X`, `OUTDOOR_Y`, `QUANTITY`는 `V17__simplify_facility_aircon_fields.sql`에서 제거. 정수기는 `V19__simplify_facility_water_purifier_fields.sql` 기준으로 `EQUIPMENT_TYPE='정수기'` 고정, `MANUFACTURE_DATE`를 설치일로 사용하고 사용자 입력은 건물/층/X/Y 좌표와 설치일만 받음. `IMAGE_PATH`는 사용자가 업로드한 이미지 경로만 저장하며, 도면 마커 이미지는 별도 마커 전용 아이콘을 사용
...
- `ROLE_ADMIN`: 전체 접근
- `FIRE_PERM`: 권한 대상 사용자별로 `code_detail.CODE`에 로그인 ID를 하나씩 등록해 소방설비 추가/수정/삭제/점검/QR 확인 권한을 제어합니다. `CODE_NAME`은 사용자 표시명으로 관리하며 `EXTRA_VALUE1`은 사용하지 않습니다.
- `OTHER_PERM`: 권한 대상 사용자별로 `code_detail.CODE`에 로그인 ID를 하나씩 등록해 기타시설관리 권한을 제어합니다. `CODE_NAME`은 사용자 표시명으로 관리하며 `EXTRA_VALUE1`은 사용하지 않습니다. 등록된 사용자만 에어컨/정수기 추가, 삭제, 좌표 이동/변경, QR 확인, 정수기 점검/이력 관리 및 에어컨 점검 요청 버튼과 API를 사용할 수 있습니다.
...
8. 권한이 있는 사용자는 추가/수정/삭제/정수기 점검/에어컨 점검 요청 및 이미지 업로드를 수행할 수 있습니다. 에어컨 ID는 `AC-000001` 형식으로 자동 생성되며, 현장 식별 No.는 별도 선택값입니다. 모바일 QR 최초 등록에서 식별 No.를 비우면 공란으로 저장됩니다. 에어컨 목록의 관리 버튼은 기존 `점검` 대신 `점검 요청`으로 표시되며 접수자 이름, 소속, 고장내용만 입력합니다. 정수기는 등록/수정 시 설치일, 건물, 층, X/Y 좌표만 입력하면 되며 종류와 기존 `WP-000001` 형식 정수기 ID는 자동으로 저장됩니다.
...
- **화면**: `설비관리시스템` 사이드바 하단에 `안전작업 매뉴얼` 그룹 메뉴로 노출됩니다(플랫폼 SPA가 `/safety/`로 시작하는 메뉴 URL을 iframe으로 로드).
  - `/safety/index.html` — **매뉴얼(이전 upload.html)는 삭제되고 엑셀 일괄업로드가 모달(`#excelModal`)로 통합된 단일 페이지입니다.** 대분류/중분류/소분류 3단계 고정 구조로
    카드형 드릴다운 레이아웃(대분류 카드 → 중분류 카드 → 소분류 카드 → 매뉴얼 목록)으로 전면 리디자인되었습니다.
    매뉴얼 목록에는 출처(엑셀 파일명/시트명) 컬럼을 표시하지 않으며, 상세는 원본 엑셀 레이아웃과 동일한
    공정순서/위험요인/안전보호구/비고 테이블 + 단계별 사진으로 표시됩니다.
- **분류 체계(3단계 고정)**: 대분류(예: 제지/화장지/패드) → 중분류(예: 3호기/4호기/5호기) → 소분류(예: 설비/안전/작업/원료)의
  딱 3단계로 강제되며, 모든 레벨이 관리자에 의해 자유롭게 추가/수정 가능합니다. **매뉴얼은 반드시 소분류(3단계)에만 등록**할
  수 있어 업로드/등록 시 서버가 강제합니다.
- **API**: `/safety-api/**` (분류/매뉴얼/단계/사진 CRUD + 엑셀 업로드, `/categories/children` 드릴다운 도우미 포함). 상세 endpoint 목록은 `sql/module-safety/README.md` 참고.
- **엑셀 일괄업로드는 2단계** (모달 단일 UI):
  1. 형식 확인(미리보기, DB 미반영) → 시트별 인식 여부/제목/단계 수/사진 수를 보여줌. 인식된 시트는 기본적으로 체크되어 있으며
     사용자는 확인/해제만 하게 됩니다. `공정단계` 헤더를 가진 개요/범례 시트(예: 초지)는 자동으로 업로드 대상에서 제외됩니다.
     모달 안에서 대/중/소분류를 선택하거나, 각 레벨의 "+" 버튼으로 새 분류를 바로 추가할 수 있습니다.
  2. 확정 업로드 → 선택한 소분류에 선택한 시트만 실제 매뉴얼로 저장(단계+사진 포함).
- **권한**: 조회는 로그인 사용자 누구나, 등록/수정/삭제/업로드는 관리자만 가능합니다. 관리자 판정은 플랫폼 `ROLE_ADMIN` 또는 공통코드 `SAFETY_PERM` 등록자입니다.
- **데이터**: `safety_manual_category`(분류, 자기참조 트리, `LEVEL_NO`로 3단계 강제) / `safety_manual`(매뉴얼, 소분류에만 연결) / `safety_manual_step`(단계) / `safety_manual_step_photo`(사진, 메타데이터만 DB에 저장하고 실제 파일은 `safety.upload-dir` 디스크 경로에 저장).
- **SQL**: `sql/module-safety/01_schema.sql` → `02_menu.sql` → `03_perm_code.sql` → `04_data.sql` → `05_category_level.sql` 순서로 실행. **반드시 `mysql --default-character-set=utf8mb4` 옵션을 붙여 실행할 것**(누락 시 한글 mojibake 발생 — 자세한 내용은 `sql/module-safety/README.md` 참고).
...
- **Last Updated**: 2026-09-02 (module-safety: 엑셀 업로드 모달화, 분류 3단계(대/중/소) 재설계, UI 전신)
...
- QR코드 화면의 좌측 설비 선택 카드 안에 `소방설비`/`기타설비` 전환 버튼을 추가했습니다. `소방설비` 버튼은 `FIRE_PERM`의 사용자 코드, `기타설비` 버튼은 `OTHER_PERM`의 사용자 코드 기준으로만 표시합니다. `소방설비` 선택 시 기존 소화기·소화전·수신기·소방펌프·스프링클러 목록을 유지하고, `기타설비` 선택 시 에어컨·정수기 등록 설비 QR을 실제 목록으로 표시합니다. 에어컨/정수기 QR은 외부 업체용 모바일 점검 페이지로 연결됩니다.
- `index.html`에서 기타설비 하위에 프론트에서만 추가되던 대시보드/도면/층별도면 가상 메뉴 및 라우팅을 제거하고, 기타설비 QR 메뉴(`OTHER_QR`) 라우팅을 `/facility/qr`로 추가
- 에어컨 식별 No. 및 제조사/위치/실외기 대수 관리 구현 완료: 모바일 QR 등록에서는 식별 No.를 선택 입력으로 처리하고, 미입력 시 `NULL`로 저장해 화면에서 공란으로 표시합니다. `V34__allow_null_facility_aircon_serial_number.sql`로 기존 DB의 `SERIAL_NUMBER` nullable 변경을 반영합니다. 실외기 좌표/연결선, 설치연도, 수량 입력은 `V17__simplify_facility_aircon_fields.sql` 기준으로 제거
...
- `V31__add_facility_mobile_qr_workflows.sql`로 에어컨 점검 요청/QR 점검(`facility_aircon_fault_report`)과 정수기 QR 점검(`facility_water_disinfection`) 테이블을 추가했습니다. `/facility-api/mobile/**` 및 `/minspection/air-conditioners|water-purifiers/{qrKey}` 모바일 QR 업무 흐름을 구현 완료. PC 에어컨 점검 요청 항목은 접수자 이름, 소속, 고장내용을 유지하고, 모바일 QR 점검 항목은 에어컨 점검자 이름+정상/비정상, 정수기 점검자 이름+완료/미완료만 받도록 단순화했습니다. 미등록 QR 최초 등록 POST는 로그인 JWT가 필요하고, 등록 화면은 도면 터치 좌표 선택을 지원합니다. 구버전 `REPORTER_PHONE`, `PHOTO_PATH` 컬럼 보정 SQL을 포함합니다.
- `V32__add_other_facility_admin_code_group.sql`로 추가된 `OTHER_PERM` 권한은 `V38__migrate_other_permission_to_user_codes.sql` 기준으로 사용자별 코드값을 사용합니다. 기타설비 목록/층별 도면/상세 모달/QR 목록의 관리 버튼 및 mutation API는 활성 `OTHER_PERM.CODE`와 로그인 ID가 일치할 때만 사용할 수 있습니다. 에어컨/정수기 상세 모달은 사진 섹션을 항상 표시하며, QR코드 이미지는 기타시설관리 권한자에게만 `/fire-api/qr/image?type=aircon|water&id={qrKey}`로 노출합니다.
- `V33__add_facility_aircon_equipment_code.sql`로 에어컨 전용 자동 ID 컬럼(`facility_equipment.EQUIPMENT_CODE`)을 신규 버전 SQL에 분리했습니다. 에어컨은 `AC-000001` 형식 ID와 식별 No.를 별도로 관리하고, 정수기는 기존 `SERIAL_NUMBER` 기반 `WP-000001` 자동 생성 정책만 유지합니다.
- QR 관리 페이지의 미등록 QR 생성 범위를 기타설비까지 확장했습니다. 에어컨/정수기 미등록 QR은 각각 `/minspection/air-conditioners/{qrKey}`, `/minspection/water-purifiers/{qrKey}` 모바일 등록 페이지로 연결되고, 등록 API는 전달받은 QR 키를 `facility_equipment.QR_KEY`에 그대로 저장합니다. 미등록 QR 최초 등록은 로그인 후에만 가능하고, 등록 대표사진 업로드는 유지됩니다. QR 점검 페이지의 사진 업로드 입력은 제거했습니다.

## Recommended Next Steps
- 실제 사용자 계정별 역할 부여 후 `ROLE_ADMIN`, `ROLE_FACILITY_MANAGER`, `ROLE_FIRE_MANAGER`, `ROLE_EQUIPMENT_MANAGER` 메뉴 노출 및 API 권한 검증
- 운영 DB에 `V17__simplify_facility_aircon_fields.sql`, `V19__simplify_facility_water_purifier_fields.sql`, `V20__fix_facility_role_duplicates.sql`, `V21__normalize_facility_menu_structure.sql`, `V22__add_new_map_zone_buildings.sql`, `V23__add_fluidized_bed_incinerator_map_zone.sql`, `V24__add_new_incinerator_map_zone_buildings.sql`, `V25__add_pad_tent_warehouse_map_zone_building.sql`, `V26__add_fire_sprinkler.sql`, `V27__fix_fire_sprinkler_menu_name_and_legacy_route.sql`, `V28__add_fire_sprinkler_qr_mobile_fields.sql`, `V29__add_other_equipment_qr_menu.sql`, `V30__move_fire_qr_to_facility_root.sql`, `V31__add_facility_mobile_qr_workflows.sql`, `V32__add_other_facility_admin_code_group.sql`, `V33__add_facility_aircon_equipment_code.sql`, `V34__allow_null_facility_aircon_serial_number.sql`, `V37__migrate_fire_permission_to_user_codes.sql`, `V38__migrate_other_permission_to_user_codes.sql` 순서 적용 후 메뉴관리/접근권한, 신규 도면 구역, 스프링클러 목록/QR/모바일 점검, 기타설비 QR 메뉴 및 에어컨/정수기 모바일 QR 업무 저장 검증
