...
- **Main Features**:
  ...
  - 수신기/소방펌프 수정 모달에서 최근 점검 이력 추가/수정/삭제 및 점검사진 업로드/변경 지원
...
10. 수신기/소방펌프 수정 모달의 점검 이력 영역에서 기존 내역은 `수정` 또는 `삭제`할 수 있고, 신규 행은 `추가`할 수 있습니다. 기존/신규 점검 이력 모두 사진 파일을 선택해 점검사진을 업로드하거나 변경할 수 있습니다.
...
- 수신기/소방펌프 수정 모달의 점검 이력 사진 업로드 입력을 추가하고, 일반 점검 저장 시 선택 사진이 생성된 점검 이력에 연결되도록 업로드 대상 식별 로직 개선 완료
